...
(function($) {
	...
	var DEFAULTS = {
        botLogoUrl: 'http://localhost:62801/assets/jquery.fbmessenger-develop/demo/logo.png',
		botName: 'jQuery.fbMessenger',
		botCategory: 'רגולציה מונעת',
	...
	Plugin.prototype._handleResize = function() {
        ...
        var base_font_size = 10;//24
		// Update font size
        var fontSize = Math.floor(width / 750 * base_font_size);
	...
	Plugin.prototype.showQuickReplies = function(quickReplies, options) {
		if (options === undefined || options.delay === undefined) {
            ...
            $.each(quickReplies, function (index, quickReply) {
                $quickReplies.append('<div  id="reply_option_' + index + '" class="jsm-quick-reply-option" style="transition-delay: ' + (index * 0.1 * timeScale).toFixed(1) + 's">' + quickReply + '</div>');
			});
    ...
    Plugin.prototype.clear_scripts = function () {
        while (this.options.script.length > 0) {
            this.options.script.pop();
        }
        this.options.state.quickRepliesDisplayed = false;
    }
