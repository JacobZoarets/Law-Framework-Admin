(function ($) {
    ...
    function OrgChart($container, opts) {
        ...
        this.draw = function () {
            ...
            $container.find('.org-edit-button').click(function (e) {
                var thisId = $(this).parent().attr('node-id');
                console.log('thisId ' + thisId);
        ...
        this.newNode = function (parentId, html_text, node_type) {
            var nextId = Object.keys(nodes).length;
            while (nextId in nodes) {
                nextId++;
            }

            self.addNode({ id: nextId, name: node_type, parent: parentId, description: html_text });
        }
    ...
    function Node(data) {
        ...
        this.formatNode = function (opts) {
            ...
            var contentHtml = "<div>s</div>";
            if (typeof data.image_url !== 'undefined' ) {
                contentHtml = getHtmlByNodeType_HeroCard(this.data.id, this.data.title, this.data.subtitle, this.data.text, this.data.image_url, this.data.media_url, this.data.button_action_1_text, this.data.button_action_1_value);
            } else if (typeof data.text !== 'undefined') {
                contentHtml = getHtmlByNodeType_text(this.data.text)
            } else if (typeof data.option1 !== 'undefined') {
                contentHtml = getHtmlByNodeType_quick_replys(this.data.option1, this.data.option2, this.data.option3)
            } else if (typeof data.name !== 'undefined') {
                contentHtml = getHtmlByNodeType(this.data.name, 0);
            }

            return "<div class='node' node-id='" + this.data.id + "'>" + nameString + buttonsHtml + contentHtml + "</div>";
        }
    }
